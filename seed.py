...
with app.app_context():
    ...
    if admin_email and admin_password and not User.query.filter(func.lower(User.email) == admin_email.lower()).first():
        admin=User(name="Administrador",email=admin_email.lower(),role="admin",must_change_password=True); admin.set_password(admin_password); db.session.add(admin)
    if customer_email and customer_password and not User.query.filter(func.lower(User.email) == customer_email.lower()).first():
        customer=User(name="Cliente Demonstração",email=customer_email.lower(),role="cliente",client_id=client.id,must_change_password=True); customer.set_password(customer_password); db.session.add(customer)
